...
# Force UTF-8 output on Windows (avoids UnicodeEncodeError on emoji/arrows in console)
...
def format_status_message(sig_4h: dict, sig_1h: dict) -> str:
    """Build a status update message sent every run, regardless of signal."""
    now_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    signal_emoji = {"BUY": "🟢", "SELL": "🔴", "NEUTRAL": "⚪", "ERROR": "⛔"}

    def render_status_block(sig: dict) -> str:
        tf = sig.get("timeframe", "?").upper()
        s = sig.get("signal", "ERROR")
        emoji = signal_emoji.get(s, "⚪")
        if s == "ERROR":
            return f"<b>[{tf}]</b> {emoji} <b>ERROR</b>: <code>{sig.get('error', 'Unknown error')[:150]}</code>"
        return (
            f"<b>[{tf}]</b> {emoji} <b>{s}</b>\n"
            f"  💰 Giá: <b>{fmt_price(sig.get('price'))}</b> /oz\n"
            f"  📊 RSI(14): {sig.get('rsi', 'N/A')}\n"
            f"  📈 EMA(9)/RSI: {sig.get('ema_rsi', 'N/A')}\n"
            f"  📉 WMA(45)/RSI: {sig.get('wma_rsi', 'N/A')}"
        )

    lines = [
        "📊 <b>Status Update — GOLD (GC=F)</b>",
        f"🕐 {now_utc}",
        "",
        render_status_block(sig_4h),
        "",
        render_status_block(sig_1h),
    ]
    return "\n".join(lines)
...
def main():
    ...
    if sig_4h.get("signal") == "ERROR":
        had_error = True
        try:
            send_telegram(bot_token, chat_id, f"⚠️ <b>Gold Egan Alert lỗi</b>\n[4H] {sig_4h.get('error')}")
        except Exception:
            pass
    ...
    # Always send a status update, every run, regardless of signal
    status_msg = format_status_message(sig_4h, sig_1h)
    status_ok = send_telegram(bot_token, chat_id, status_msg)
    print(f"[INFO] Status update → Telegram {'sent' if status_ok else 'FAILED'}")

    # Only send prominent real-signal alert if at least one timeframe has a non-NEUTRAL, non-ERROR signal
    actionable_signals = {sig_4h.get("signal"), sig_1h.get("signal")} - {"NEUTRAL", "ERROR", None}

    if not actionable_signals:
        print("[INFO] Both timeframes NEUTRAL — no real-signal alert sent.")
    else:
        print(f"[INFO] Actionable signals detected: {actionable_signals} — sending Telegram alert...")
        message = build_message(sig_4h, sig_1h)
        message = f"🚨 <b>TÍN HIỆU BUY/SELL THẬT</b> 🚨\n\n{message}"
